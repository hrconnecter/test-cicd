...
const BirthdaySection = ({ birthdays }) => {
    ...
    useEffect(() => {
        if (!birthdays || birthdays.length === 0) {
            return; // Skip animation if there are no birthdays
        }
        ...
        const step = (timestamp) => {
            ...
            if (fadeOutStartTime) {
                ...
                const fadeDuration = 1000; // Make fade duration longer
                const fadeProgress = Math.min(elapsed / fadeDuration, 1); // Smooth fade progress

                // Adjust the opacity decrease speed
            ...
            // Stop the animation when the fade is complete
            if (fadeOutStartTime && Date.now() - fadeOutStartTime >= 1000) { // Match fade duration
        ...
        >
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
                <h1 className="flex text-[20px] font-semibold text-[#67748E] my-2">
                    <FaCakeCandles
                        style={{ color: "blue", fontSize: "24px", margin: "5px 10px" }}
                    />
                    Upcoming Birthdays
                </h1>
                        ...
                        {birthdays.map((birthday, index) => (
                            ...
                            >
                                <Avatar
                                    ...
                                    src={birthday.user_logo_url || "/placeholder-image.png"}
                                />
                                <Typography
                                    variant="p"
                                ...
                                >
                                    <Typography sx={{ color: "black", textAlign: "center" }}>
                                        {birthday.date_of_birth
                                            ? format(new Date(birthday.date_of_birth), "d MMM")
                                            : "-"}
                                    </Typography>
                                </div>
                            </Box>
