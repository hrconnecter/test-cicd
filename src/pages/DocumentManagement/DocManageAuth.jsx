...
const DocManageAuth = () => {
  ...
  // Helper function to determine if document is editable
  ...
  try {
    ...
    setTimeout(() => {
      if (quillRef.current && quillRef.current.getEditor) {
        ...
        editor.clipboard.dangerouslyPasteHTML(document.details || '');
      }
    }, 100);
    ...
    if (document.empidd) {
      ...
      try {
        ...
        if (empResponse.data && empResponse.data.employees && Array.isArray(empResponse.data.employees)) {
          // Find the employee with matching ID
          const matchingEmployee = empResponse.data.employees.find(emp => emp._id === document.empidd);
...
// New handler for selecting non-editable documents
...
// Handler for sending document to manager
...
{/* Status indicator for better understanding */}
{docId && (
  <div className="mb-4">
    <div className={`px-3 py-2 rounded-md ${
      data2?.find(doc => doc._id === docId)?.docstatus === "To Do" 
        ? "bg-yellow-50 border border-yellow-200" 
        : data2?.find(doc => doc._id === docId)?.docstatus === "Pending" 
          ? "bg-blue-50 border border-blue-200"
          : data2?.find(doc => doc._id === docId)?.docstatus === "Accepted"
            ? "bg-green-50 border border-green-200"
            : data2?.find(doc => doc._id === docId)?.docstatus === "Send"
      ...
      <div className="flex items-center">
        ...
        <Typography variant="body2" className="font-medium">
          Status: <span className="font-bold">{data2?.find(doc => doc._id === docId)?.docstatus || "Unknown"}</span>
        </Typography>
      </div>
      <Typography variant="caption" className="block mt-1 ml-5 text-gray-600">
        {data2?.find(doc => doc._id === docId)?.docstatus === "To Do" 
                ...
                <>
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      ...
                      className={`block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 ${!isDocumentEditable() ? 'bg-gray-50' : 'bg-gray-100'}`}
...
{/* euu */}
                  ...
                  <div className="mb-4">
                    <TextField
                      ...
                      fullWidth
                      margin="normal"
                      className={`${!isDocumentEditable() ? 'bg-gray-50' : 'bg-gray-100'}`}
                    />
...
<ReactQuill
 ref={quillRef}
    ...
    theme={isDocumentEditable() ? "snow" : "bubble"} // Use bubble theme for read-only
    value={newDocument.details || ''} // Ensure there's always a value, even if empty
    onChange={(value) =>
      setNewDocument((newDocument) => ({
        ...newDocument,
        details: value,
    ...
    readOnly={!isDocumentEditable()} // Make it read-only when not editable
    modules={{
      toolbar: isDocumentEditable() ? [
        [{ font: [] }],
        ...
        [{ align: [] }],
        ["clean"],
      ] : false, // Hide toolbar when not editable
    }}
    style={{ width: "100%" }}
    preserveWhitespace={true} // Add this to preserve whitespace
  />
              ...
              <div className="flex gap-2 mt-3 justify-center">
                {!docId ? (
                  ...
                  <>
                    ...
                    {isDocumentEditable() && 
                     letterWorkflow[selectedLetterType]?.workflow === true && (
                      <Button
                        variant="contained"
                        size="small"
                    ...
                    {((isDocumentEditable() && 
                       letterWorkflow[selectedLetterType]?.workflow === false) || 
                      data2?.find(doc => doc._id === docId)?.docstatus === "Accepted") && (
                      <Button
                        variant="contained"
                        size="small"
...
//         doc.setFontSize(18);
//         doc.setTextColor(0, 102, 204);
//         doc.text(`Document Management`, pageWidth / 2, margin + 10, null, null, "center");
  
//         doc.setFontSize(12);
//         doc.setTextColor(0, 0, 0);
//         doc.text(`${docType}`, pageWidth / 2, margin + 20, null, null, "center");
...
//         doc.text(`Title: ${docTitle}`, pageWidth / 2, margin + 35, null, null, "center");
//       }
//     };
