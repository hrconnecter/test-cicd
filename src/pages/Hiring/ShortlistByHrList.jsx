...
const ShortlistByHrList = () => {
  ...
  const { data, isLoading } = useQuery(
    ...
    async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API}/route/organization/${organisationId}/job/${vacancyId}/shortlisted-applications/hiring`,
  ...
  const handleViewDetails = (vacancy) => {
    console.log("vacancy in hiring", vacancy);

    if (vacancy?._id) {
      navigate(
        `/organisation/${organisationId}/view-job-detail-application-hiring/${vacancy?._id}`
      );
    } else {
