...
const MrOpenJobVacancyList = () => {
  ...
  const { data, isLoading } = useQuery(
    ["JobVacancyHiring", organisationId],
    async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API}/route/organization/${organisationId}/manager/vacancies/hiring`,
        {
          headers: {
  ...
  const editVacancy = (vacancyId) => {
    navigate(
      `/organisation/${organisationId}/manager-create-job-position/edit/${vacancyId}`
    );
  };
  ...
  const handleShortList = (vacancyId) => {
    navigate(
      `/organisation/${organisationId}/hr-shortlisted-hiring/${vacancyId}`
    );
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      queryClient.invalidateQueries(["JobVacancyHiring", organisationId]);
    } catch (err) {
    ...
    <div>
      ...
      <div className="overflow-auto  bg-white  mt-2">
        {isLoading ? (
                ...
                <tr key={vacancy._id} className="border-b hover:bg-gray-100">
                  ...
                  <td className="pl-8 py-3">
                    <BasicButton
                      variant="outlined"
                      title="shortlist"
                      onClick={() => handleShortList(vacancy._id)}
                    />
                  </td>
                </tr>
              ))}
          ...
          <div style={{ padding: "20px" }}>Job vacancy not found.</div>
        )}
      </div>
