...
const Test2 = ({ isLastStep, nextStep, prevStep }) => {
  ...
  //       handleAlert(true, "warning", "Please upload a valid PDF file.");
  //       return;
  //     }
  //     try {
  ...
  //       handleAlert(true, "error", "File upload failed. Please try again.");
  //       return;
  //     }
  //   } else {
  ...
  const onSubmit = async (data) => {
    ...
    if (isFileObject && !vacancyData?.additionalDocument) {
      if (file.type !== "application/pdf") {
        handleAlert(true, "warning", "Please upload a valid PDF file.");
        return;
      }
      ...
      // Either file is a URL or already uploaded
      fileUrl =
        typeof file === "string" ? file : vacancyData?.additionalDocument;
    }
  ...
  useEffect(() => {
    if (vacancyData) {
      ...
      if (vacancyData?.additionalDocument) {
        setValue("additionalDocument", vacancyData.additionalDocument);
      }
    }
      ...
      >
        <Grid container>
          ...
          <Grid item xs={12} md={12}>
            ...
            />
            {(vacancyData?.additionalDocument ||
              typeof watch("additionalDocument") === "string") && (
              <div className="mt-2">
                <a
                  href={
                    typeof watch("additionalDocument") === "string"
                  ...
                  className="flex text-blue-500 gap-2"
                >
                  <FaRegEye className="my-1" /> View Previously Uploaded
                  Document
                </a>
                <p className="text-sm text-gray-500">
                  Upload a new document to replace the existing one.
