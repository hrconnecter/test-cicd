...
//         try {
//             const response = await axios.get(
//                 `${process.env.REACT_APP_API}/route/foundation-attendance`,  // Fixed API route (removed extra "/")
//                 {
//                     params: {
//                         employeeId: userId,
//                         startDate,
...
//     useEffect(() => {
//         console.log("Updated attendanceData:", attendanceData?.totalWorkingHours);
//         setWorkingHours(attendanceData?.totalWorkingHours || 0);
//     }, [attendanceData]);  // Runs when attendanceData updates
...
//     console.log(" FY Getting financial year range with format:", yearFormat, "for date:", selectedDate.format("YYYY-MM-DD"));
//     switch(yearFormat) {
//       case 'jan-dec':
...
//     const selectedYear = selectedDate.year();
//     const selectedMonth = selectedDate.month() + 1;
//     console.log("FY Calculating financial year period with format:", yearFormat, "for date:", selectedDate.format("YYYY-MM-DD"));
...
//     const result = {
//       startDate: dayjs(`${startYear}-${startMonth.toString().padStart(2, '0')}-01`),
//       endDate: dayjs(`${endYear}-${endMonth.toString().padStart(2, '0')}-${dayjs(`${endYear}-${endMonth}-01`).daysInMonth()}`)
//     };
...
//     console.log("FY Filtering salaries between:", startDate.format("YYYY-MM-DD"), "and", endDate.format("YYYY-MM-DD"));
//     console.log("FY Total salaries before filtering:", salaries.length);
    
//     const filteredSalaries = salaries.filter(salary => {
//       const salaryDate = dayjs(`${salary.year}-${salary.month.toString().padStart(2, '0')}-01`);
//       const isInRange = (salaryDate.isAfter(startDate) || salaryDate.isSame(startDate)) && 
//                        (salaryDate.isBefore(endDate) || salaryDate.isSame(endDate));
      
//       console.log("FY Salary date:", salaryDate.format("YYYY-MM-DD"), "In range:", isInRange);
...
//     console.log("Calculating annual totals from submitted salaries:", submittedSalaries);
    
//     const currentFYSalaries = filterSalariesForCurrentFinancialYear(submittedSalaries);
    
//     console.log("FY Filtered salaries for current financial year:", currentFYSalaries);
...
//     const expenseAmount = expensePayments?.reduce((total, expense) => 
//       total + (expense.isReport ? expense.totalAmount : expense.amount), 0) || 0;
    
//     // In your existing salary calculation code, modify only the necessary parts:
//     const totalIncome = isdailywage ? (incomeValues?.reduce((a, c) => a + c.value, 0)) * workingHours : incomeValues?.reduce((a, c) => a + c.value, 0) || 0;
//     const totalDeduction = deductionValues?.reduce((a, c) => a + c.value, 0) || 0;
...
//     designation: (availableEmployee?.designation &&
//       availableEmployee?.designation.length > 0 &&
//       availableEmployee?.designation[0]
//         ?.designationName) ||
//       "",
//     department: (availableEmployee?.deptname &&
...
//   const monthlyIncomeData = incomeValues
...
//   const monthlyDeductionData = deductionValues?.map((deduction) => ({
...
//   const annualIncomeData = incomeValues?.map((income) => ({
//     particular: income.name,
//     amount: submittedSalaries?.length > 0 ? Math.round(income.value * submittedSalaries.length) : 0,
//   })) || [];
  
//   // const annualDeductionData = [{ particular: "", amount: "0" }];
//   const annualDeductionData = deductionValues?.map((deduction) => ({
//     particular: deduction.name !== "Loan Deduction" ? deduction.name : "\u00A0", // Exclude "Loan Deduction"
//     amount:
...
//   const monthlyTotals = { grossSalary: salary?.totalIncome || "", totalDeduction: salary?.totalDeduction || "", netSalary: salary?.totalNetSalary || "" };
  
//   const annualTotal = { grossSalary: salary?.totalIncome
//     ? Math.round(
//         salary?.totalIncome *
