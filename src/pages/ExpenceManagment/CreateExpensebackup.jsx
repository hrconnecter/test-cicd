...
const CreateExpense = () => {
  ...
  // const ReportExpenseSchema = z.object({
  //   reportDetails: z
  //     .object({
  //       reportName: z.string().min(1, "Report name is required"),
  //       reportDate: z.string().min(1, "Report date is required"),
  //       reportType: z.object({
  //         value: z.string().min(1, "Report type is required"),
  //         label: z.string().min(1, "Report type is required"),
  //       }),
  //       reportFor: z
  //         .object({
  //           value: z.string(),
  ...
  //   resolver: zodResolver(isReportMode ? ReportExpenseSchema : ExpenseSchema),
  //   defaultValues: {
  //     expenseName: "",
  ...
  // const handleFileExpenseUpload = async (file) => {
  //   try {
  ...
  //     return {
  //       name: file.name,
  //       url: url.split("?")[0], // Clean URL without signature
  //       key: key,
  //       type: file.type
  //     };
  //   } catch (error) {
  //     console.error("Error uploading file:", error);
  //     handleAlert(true, "error", "Failed to upload attachment");
  //     throw error;
  //   }
  // };
  ...
  //     handleAlert(true, "error", error.message || "Failed to upload attachments");
  //     throw error;
  //   }
  // };
  ...
  //     return []; // No files, return empty array
  //   }
  ...
  //       attachments = await handleFileExpenseUpload(expenseData.attachments);
  //     }

  //     const newExpense = {
  //       ...expenseData,
  //       attachments, // Store uploaded URLs
  //     };
  ...
  //       // Update total
  //       setReportTotal(calculateReportTotal([...expenses, newExpense]));
  ...
  //     });
  //   } catch (error) {
  //     handleAlert(true, "error", "Failed to add expense. Please try again.");
  //   } finally {
  ...
  // const handleAddExpense = async (expenseData) => {
  //   setIsSubmitting(true);
  //  let  attachments = await handleFileExpenseUpload(expenseData.attachments)
  //   try {
  //     let attachments = [];
  //     console.log(expenseData)
  //     if (expenseData.attachments?.length > 0) {
  //       // Create a Set of files based on name to ensure uniqueness
  //       const uniqueFiles = Array.from(expenseData.attachments).filter((file, index, self) =>
  //         index === self.findIndex((f) => f.name === file.name)
  //       );

  //       // Upload each unique file
  //       // attachments = await Promise.all(
  //       //   uniqueFiles.map(file => handleFileExpenseUpload(file))
  //       // );
  //     }

  //     const newExpense = {
  //       ...expenseData,
  //       attachments // Single array of unique attachments
  //     };
  ...
  //     // Reset form while preserving report details
  //     const currentReportDetails = methods.getValues("reportDetails");
  //     methods.reset({
  //       reportDetails: currentReportDetails,
  //     });
  //   } catch (error) {
  //     handleAlert(true, "error", "Failed to process attachments");
  //   } finally {
  //     setIsSubmitting(false);
  //   }
  // };
  ...
  //   if (!formValid) {
  //     handleAlert(true, "error", "Please fill all required fields correctly");
  //     return;
  //   }
  //   setIsSubmitting(true);
  //   try {
  //     let attachments = await handleFileExpenseUpload(values.attachments);
  ...
  //       ...values,
  //       amount: parseFloat(values.amount),
  //       attachments, // This will be either an empty array or uploaded files
  //     };
  ...
  //friday
  // Modify the handleAddExpense function to include validation
// Modify the handleAddExpense function to validate only expense fields
// const handleAddExpense = async (values) => {
//   // Create an array of expense-specific fields to validate
//   const expenseFields = [
//     'expenseName',
//     'category',
//     'amount', 
...
//   // Trigger validation only for expense fields
//   const formValid = await methods.trigger(expenseFields);
...
//     // Reset only expense fields while preserving report details
//     const currentReportDetails = methods.getValues("reportDetails");
//     methods.reset({
//       reportDetails: currentReportDetails,
//     });
...
// };
  ...
  //   // Preserve report details while setting expense form data
  //   const currentReportDetails = methods.getValues("reportDetails");
  ...
  //     approver: expenseToEdit.approver,
  //     region: expenseToEdit.region,
  //     relatedEvent: expenseToEdit.relatedEvent,
  ...
  const createReportMutation = useMutation(
    async ({ reportDetails, expenses, isDraft = false }) => {
      ...
      // Ensure proper expense object structure
      const processedExpenses = expenses.map((expense) => ({
  ...
  //   // setReportTotal(calculateReportTotal(draft.expenses));

  //   methods.reset({
  //     reportDetails: {
  //       reportName: draft.reportName,
  //       reportDate: draft.reportDate.split("T")[0],
  //       reportType: {
  //         value: draft.reportType,
  //         label: draft.reportType,
  //       },
  //       reportFor: draft.reportFor && {
  //         value: draft.reportFor._id,
  //         label: `${draft.reportFor.first_name} ${draft.reportFor.last_name}`,
  //       },
  //       reportApprover: draft.approver && {
  //         value: draft.approver._id,
  //         label: `${draft.approver.first_name} ${draft.approver.last_name}`,
  //       },
  //     },
  //   });
  //   // Load expenses from draft
  ...
  //       label: `${expense.approver.first_name} ${expense.approver.last_name}`,
  //     },
  //     region: expense.region,
  //     attachments: expense.attachments,
  //     relatedEvent: expense.relatedEvent || "",
  ...
  //   // Set report details with null checks
  //   methods.reset({
  //     reportDetails: {
  //       reportName: draft.reportName || '',
  ...
  //   // Format expenses with null checks
  //   const formattedExpenses = draft.expenses.map((expense) => ({
  //     expenseName: expense.expenseName || '',
  //     // category: {
  //     //   value: expense.category?._id || expense.category?.value || '',
  //     // label: expense.category?.name || expense.category?.label || '',
  //     // },

  //     // category: typeof expense.category === 'string'
  //     // ? { value: expense.category, label: '' } // Use empty label since name is missing
  //     // : {
  //     //     value: expense.category?._id || expense.category?.value || '',
  //     //     label: expense.category?.name || expense.category?.label || '',
  //     //   },
  ...
  //   ? { value: expense.category, label: expense.category } // Use ID as label temporarily
  //   : {
  //       value: expense.category?._id || expense.category?.value || '',
  //       label: expense.category?.name || expense.category?.label || '',
  //     },
  ...
  const handleEditReportDraft = (draft) => {
    ...
    const formattedExpenses = draft.expenses.map((expense) => {
      ...
      const matchingCategory = categoryOptions.find(cat => cat.value === expense.category);
  
      return {
        expenseName: expense.expenseName || '',
        category: matchingCategory || {
          value: expense.category,
          label: categoryOptions.find(cat => cat.value === expense.category)?.label || ''
        },
        amount: (expense.amount || 0).toString(),
        startDate: expense.startDate ? expense.startDate.split("T")[0] : '',
        endDate: expense.endDate ? expense.endDate.split("T")[0] : '',
      ...
      <div className="flex flex-col justify-between w-full md:ml-4">
        <div className="flex justify-between items-center mb-4">
          ...
          {/* <button
            ...
            >
              {isReportMode ? "Switch to Single Expense" : "Create Report"}
            </button> */}
        </div>
...
/> */}
  ...
  className="w-full bg-primary text-white py-2 rounded-md hover:bg-secondary"
>
  {editingExpenseIndex !== null ? "Update Expense" : "Add Expense to Report"}
</button> */}
                    </div>
                  </div>
                </div>
 ...
 */}
                ...
                {/* {methods.watch("attachments")?.map((file, index) => (
...
//       const updatedExpenses = [...expenses];
//       updatedExpenses[editingExpenseIndex] = expenseData;
//       setExpenses(updatedExpenses);
//       setEditingExpenseIndex(null);
//     } else {
//       // Add new expense
...
//     setReportTotal(calculateReportTotal(expenses));

//     // Reset form while keeping report details
//     const currentReportDetails = methods.getValues("reportDetails");
//     methods.reset({
//       reportDetails: currentReportDetails,
//     });
...
//         attachments = await handleFileExpenseUpload(
//           data.attachments,
//           data.category.value
//         );
//       }
...
//   //           let attachments = [];
//   //           if (expense.attachments instanceof File) {
//   //             attachments = await handleFileExpenseUpload(
//   //               expense.attachments,
...
//   //                 : expense.category,
//   //             approver:
//   //               typeof expense.approver === "object"
//   //                 ? expense.approver.value
//   //                 : expense.approver,
//   //             attachments: expense.attachments || [attachments],
//   //           };
//   //         })
//   //       ),
//   //       status: isDraft ? "DRAFT" : "PENDING",
...
//   //       const message = selectedDraft
//   //         ? variables.isDraft
//   //           ? "Report draft updated successfully"
...
//               // Upload to S3
//               await axios.put(url, expense.attachments, {
//                 headers: { "Content-Type": expense.attachments.type }
//               });

//               // Store attachment details
//               attachments.push({
//                 url: url.split("?")[0],
//                 name: expense.attachments.name,
//                 type: expense.attachments.type
//               });
//             } catch (error) {
//               console.error("File upload error:", error);
...
//             region: expense.region,
//             relatedEvent: expense.relatedEvent || "",
//             attachments // Using the attachments array
//           };
//         })
//       );
...
//         status: isDraft ? "DRAFT" : "PENDING",
//         totalAmount: reportTotal,
//         attachments: [] // Report level attachments
//       };
