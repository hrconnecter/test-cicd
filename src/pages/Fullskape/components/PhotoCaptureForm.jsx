...
//         if (role === "Teacher") {
//             // Skip face recognition and start directly
//             setIsFaceMatched(true);

//         } else if (employeeOrgId?.employee?.faceRecognition) {
...
//                         disabled={
//                             role === "Teacher"
//                                 ? !imageCaptured // Enable only after "Capture" is clicked for "Teacher"
...
const PhotoCaptureForm = ({ setOpen }) => {
    const { media, setStart} = useSelfieStore();
    const photoRef = useRef();
    const videoRef = useRef();
    const { UserInformation } = useHook();
    const profileImage = UserInformation?.user_logo_url;
