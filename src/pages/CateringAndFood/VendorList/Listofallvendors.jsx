...
const Listofallvendors = () => {
  ...
  console.log("authTokenak", authToken);
  console.log("organisationId", organisationId);
  const fetchAvailableEmployee = useCallback(
    async (organisationId, authToken, page) => {
      try {
        setIsLoading(true);
        const apiUrl = `${process.env.REACT_APP_API}/route/employee/vendor/get-paginated-vendor/${organisationId}?page=${page}&nameSearch=${debouncedNameSearch}&deptSearch=${debouncedDeptSearch}&locationSearch=${debouncedLocationSearch}&sortBy=${sortBy}&sortOrder=${sortOrder}`;
  ...
  const deleteMutation = useMutation(
      ...
      }),
    {
      onSuccess: () => {
        ...
        handleAlert(true, "success", "Vendor deleted succesfully");
      },
    }
  );
  ...
  const handleEditClick = (empId) => {
    navigate(`/organisation/${organisationId}/edit-vendor/${empId}`);
  };
  ...
  const handleDeleteMultiple = () => {
    ...
    if (selectedEmployees.length === 0) {
      handleAlert(true, "error", "Please select Vendors to delete");
      return;
    }
  ...
  const confirmDeleteMultiple = async () => {
    try {
      ...
      queryClient.invalidateQueries("vendors");
      handleAlert(true, "success", "Vendor deleted successfully");
      ...
      handleAlert(true, "error", "Failed to delete Vendors");
    } finally {
          ...
          for (const employee of employeesToDelete) {
            try {
              ...
              setAppAlert({
                alert: true,
                type: "error",
                msg: "Failed to delete vendor from excel. Please try again.",
              });
            }
          }
          handleClose();
  ...
  return (
    <>
      <BoxComponent>
        ...
        <HeadingOneLineInfo
          heading={"Vendor Offboarding"}
          info={" Delete vendor data here by using delete button"}
        />
        <div className="pb-4 border-b-[.5px] flex flex-col md:flex-row items-center justify-between gap-2 w-full border-gray-300">
          ...
          <div className="flex items-center gap-2 mb-3 md:mb-0 w-full md:w-auto">
            <TextField
              onChange={(e) =>
                handleSearchChange("companyname", e.target.value)
              }
            ...
            <tbody>
              {availableEmployee1.map((item, id) => (
                <tr className="!font-medium border-b" key={id}>
                  ...
                  <td className="py-3 pl-8">{item?.vendorId}</td>
                  <td className="py-3 pl-8">{item?.address}</td>
                  ...
                  {/* <td className="py-3 pl-8">
                    {item?.deptname?.map((dept, index) => {
                      return <span key={index}>{dept?.companyname}</span>;
                    })}
                  </td> */}
                  <td className="py-3 pl-8">{item?.companyname}</td>
        ...
        >
          ...
          <DialogContent>
            <p>
              Please confirm your decision to delete this vendor, as this action
              cannot be undone.
            </p>
          </DialogContent>
          <DialogActions>
        ...
        >
          ...
          <DialogContent>
            <p>
              Please confirm your decision to delete this selected vendor, as
